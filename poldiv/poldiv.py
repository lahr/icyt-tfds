...
_DATASET = "dataset-20210329.tar.gz"
...
class Poldiv(tfds.core.GeneratorBasedBuilder):
    ...
    def _info(self) -> tfds.core.DatasetInfo:
        ...
        if self.builder_config.selection == 'all':
            channels = {str(i + 1): tfds.features.Image(dtype=tf.uint16, shape=(None, None, 1)) for i in range(6)}
            channels['9'] = tfds.features.Image(dtype=tf.uint16, shape=(None, None, 1))
            masks = {str(i + 1): tfds.features.Image(dtype=tf.uint16, shape=(None, None, 1)) for i in range(6)}
            masks['9'] = tfds.features.Image(dtype=tf.uint16, shape=(None, None, 1))
        ...
        for filename, fobj in path_iter:
            if self.builder_config.selection == 'all':
                ...
                if num_channels == 7 or num_channels == 9:
                    channels = {str(i + 1): img[:, :, i:i + 1] for i in range(0, 6)}
                    channels['9'] = img[:, :, 6:7]
                    masks = {str(i - 6): img[:, :, i:i + 1] for i in range(7, 13)}
                    masks['9'] = img[:, :, 13:14]

                elif num_channels == 12:
                    channels = {str(i + 1): img[:, :, i:i + 1] for i in range(0, 6)}
                    channels['9'] = img[:, :, 8:9]
                    masks = {str(i - 11): img[:, :, i:i + 1] for i in range(12, 18)}
                    masks['9'] = img[:, :, 20:21]
